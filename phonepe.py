...
column_map = {
    ...
    'map_insurance': ["States", "Years", "Quarter", "Districts", "Transaction_count", "Transaction_amount"],
...
# ✅ Assign to expected names used in your Streamlit code
Aggre_insurance = dataframes['aggregated_insurance']
Aggre_transaction = dataframes['aggregated_transaction']
Aggre_user = dataframes['aggregated_user']
Map_insurance = dataframes['map_insurance']
Map_transaction = dataframes['map_transaction']
Map_user = dataframes['map_user']
Top_insurance = dataframes['top_insurance']
Top_transaction = dataframes['top_transaction']
Top_user = dataframes['top_user']
...
def Aggre_insurance_Y(df,year):
    ...
    with col1:
        ...
        st.plotly_chart(fig_amount, key=f"fig_amount_bar_{year}_{df.shape[0]}")
    with col2:
        ...
        st.plotly_chart(fig_count, key=f"fig_count_bar_{year}_{df.shape[0]}")
    ...
    with col3:
        ...
        st.plotly_chart(fig_india_1, key=f"fig_india_amount_map_{year}_{df.shape[0]}")
    ...
    with col4:
        ...
        st.plotly_chart(fig_india_2, key=f"fig_india_count_map_{year}_{df.shape[0]}")
...
def Aggre_insurance_Y_Q(df,quarter):
    ...
    with col1:
        ...
        st.plotly_chart(fig_q_amount, key=f"fig_q_amount_{quarter}_{id(df)}")
    ...
    with col2:
        ...
        st.plotly_chart(fig_q_count, key=f"fig_q_count_{quarter}_{id(df)}")
    ...
    with col3:
        ...
        st.plotly_chart(fig_india_1, key=f"fig_choropleth_amount_{quarter}_{id(df)}")
    with col4:
        ...
        st.plotly_chart(fig_india_2, key=f"fig_choropleth_count_{quarter}_{id(df)}")
...
if select == "Home":
    ...
    with col2:
        st.video("videos/PHONEPE.mp4")

    col3,col4= st.columns(2)
    
    with col3:
        st.video("videos/PhonePe_Motion_Graphics.mp4")
